...
.
    ...
    if bedroom_num == 'studio' or bedroom_num == "Studio":
        num = 1
    else:
...
def get_detailed_listing_database(html_file):
    ...
    all_info = []
    for x in range(len(first_three)):
        id = first_three[x][-1]
        #print(id)
        last_three = get_listing_information(id)
        first_three[x] += last_three
        #print(first_three[x])
        all_info.append(first_three[x])
    #print(all_info)
    return all_info
...
def write_csv(data, filename):
    ...
    """
    sort = sorted(data, key = lambda t:t[1])
    #print(sort)
    fields = ["Listing Title","Cost","Listing ID","Policy Number","Place Type","Number of Bedrooms"]
    rows = []
...
def extra_credit(listing_id):
    ...
    """
    reviews = "html_files/listing_" + listing_id + "_reviews.html"
    f = open(reviews,'r')
    content = f.read()
    f.close()
    soup = BeautifulSoup(content, 'html.parser')
